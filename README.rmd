...
Modelo de ocupação single-season single-species para espécies não marcadas de acordo com o proposto por MacKenzie et al. (2002). Segue o método **stepwise** de seleção de variáveis.
...
## Inputs
Variáveis explanatórias e tabela de detecção/não-detecção em excel.
Atenção: separação decimal por ponto, e tabela de detecção/não-detecção preenchida apenas por 0 e 1, sendo que 0 significa não-detecção, na ausência de dado (por exemplo no caso de armadilhas fotográficas com diferentes períodos de amostragem) a célula deve ser deixada vazia (nada de " - ").

## Modelagem
Após o input dos dados e preparação dos mesmos é vem o principal que é a modelagem, ela é dividida em duas etapas, na primeira é estimada a probabilidade de detectar as e usada em uma segunda etapa para estimar o parâmetro da ocupação.
...
grViz("digraph flowchart {
      ...
      [1]: 'Testar os modelos de detecção psi(.)p(.), psi(.)p(t), psi(.)p(var)'
      [2]: 'Caso seja selecionado o modelo psi(.)p(var)'
      [3]: 'Gerar modelos de detecção com base nas covariáveis'
      [4]: 'Caso o seja selecionado o modelo psi(.)p(t), ou psi(.)p(.)'
      [5]: 'Escrever modelo de detecção final'
